...
import { getInteractionHandler } from "../react/interaction.js";
import { activeFamily, containerLayoutFamily, family, focusFamily, hoverFamily, observable, VAR_SYMBOL } from "../reactivity.js";
import { calculateProps } from "./calculate-props.js";
export const stylesFamily = family((hash, rules) => {
...
export function getStyledProps(state, inline) {
  let result;
  ...
  for (const config of state.configs) {
    ...
    if (hoverFamily.has(state.ruleEffectGetter)) {
      result ??= {};
      result.onHoverIn = getInteractionHandler(state.ruleEffectGetter, "onHoverIn", inline?.onHoverIn);
      result.onHoverOut = getInteractionHandler(state.ruleEffectGetter, "onHoverOut", inline?.onHoverOut);
    }
    if (activeFamily.has(state.ruleEffectGetter)) {
      result ??= {};
      result.onPress = getInteractionHandler(state.ruleEffectGetter, "onPress", inline?.onPress);
      result.onPressIn = getInteractionHandler(state.ruleEffectGetter, "onPressIn", inline?.onPressIn);
      result.onPressOut = getInteractionHandler(state.ruleEffectGetter, "onPressOut", inline?.onPressOut);
    }
    if (focusFamily.has(state.ruleEffectGetter)) {
      result ??= {};
      result.onBlur = getInteractionHandler(state.ruleEffectGetter, "onBlur", inline?.onBlur);
      result.onFocus = getInteractionHandler(state.ruleEffectGetter, "onFocus", inline?.onFocus);
    }
    if (containerLayoutFamily.has(state.ruleEffectGetter)) {
      result ??= {};
      result.onLayout = getInteractionHandler(state.ruleEffectGetter, "onLayout", inline?.onLayout);
    }
  }
  return result;
}
